...
import com.eve.eplusweibo.slidemenu.SlidMenu.CanvasTransformer;
...
public class CustomViewBehind extends ViewGroup {
	...
	/**这个是margin模式的时候用的，margin的范围*/
	private static final int MARGIN_THRESHOLD = 48; // dips
	private int mTouchMode = SlidMenu.TOUCHMODE_MARGIN;
	...
	public void setMode(int mode) {
		if (mode == SlidMenu.LEFT || mode == SlidMenu.RIGHT) {
			if (mContent != null)
	...
	public int getMenuPage(int page) {
		...
		if (mMode == SlidMenu.LEFT && page > 1) {
			return 0;
		} else if (mMode == SlidMenu.RIGHT && page < 1) {
			return 2;
		} else {
			return page;
		}
	...
	public int getAbsLeftBound(View content) {
		if (mMode == SlidMenu.LEFT || mMode == SlidMenu.LEFT_RIGHT) {
			return content.getLeft() - getBehindWidth();
		} else if (mMode == SlidMenu.RIGHT) {
			return content.getLeft();
		}
	...
	public int getAbsRightBound(View content) {
		if (mMode == SlidMenu.LEFT) {
			return content.getLeft();
		} else if (mMode == SlidMenu.RIGHT || mMode == SlidMenu.LEFT_RIGHT) {
			return content.getLeft() + getBehindWidth();
		}
		return 0;
	}
	...
	public boolean menuTouchInQuickReturn(View content, int currPage, float x) {
		if (mMode == SlidMenu.LEFT
				|| (mMode == SlidMenu.LEFT_RIGHT && currPage == 0)) {
			return x >= content.getLeft();
		} else if (mMode == SlidMenu.RIGHT
				|| (mMode == SlidMenu.LEFT_RIGHT && currPage == 2)) {
