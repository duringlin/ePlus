...
import com.eve.eplusweibo.slidemenu.SlidMenu.OnClosedListener;
import com.eve.eplusweibo.slidemenu.SlidMenu.OnOpenedListener;
...
public class CustomViewAbove extends ViewGroup {
	...
	private static final int MAX_SETTLE_DURATION = 244; // ms
	...
	/**
	 * Determines speed during touch scrolling 控制触摸滚动的速度
	 */
	...
	protected int mTouchMode = SlidMenu.TOUCHMODE_MARGIN;
	...
	private boolean thisTouchAllowed(MotionEvent ev) {
		...
		if (isMenuOpen()) {
			...
			switch (mTouchMode) {
			case SlidMenu.TOUCHMODE_FULLSCREEN:
				return !isInIgnoredView(ev);
			case SlidMenu.TOUCHMODE_NONE:
				return false;
			case SlidMenu.TOUCHMODE_MARGIN:
	...
	public boolean onInterceptTouchEvent(MotionEvent ev) {
		// 如果return true,那么表示该方法消费了此次事件，如果return
		// false，那么表示该方法并未处理完全，该事件仍然需要以某种方式传递下去继续等待处理。
		...
		try {
			...
			e.printStackTrace();
			// 据说这里有个android自身的bug,当在屏幕上，进行多点快速滑动的时候会报这个错
			return false;
		}
	}
	...
	public void scrollTo(int x, int y) {
		...
		((SlidMenu) getParent()).manageLayers(getPercentOpen());
	}
